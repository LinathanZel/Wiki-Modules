...
dirpath = os.getcwd()
filename = "megabbs.json.txt"
bbs3_url = "https://raw.githubusercontent.com/Deathmax/bravefrontier_data/master/bbs_3.json"
bbs8_url = "https://raw.githubusercontent.com/Deathmax/bravefrontier_data/master/bbs_8.json"
errors = 0
globalArr=[]
arrCount=0
...
try:
    ...
    try:
        with urlopen(bbs3_url) as url:
            data = json.loads(url.read().decode())
        with urlopen(bbs8_url) as url:
            bbs = json.loads(url.read().decode())
        for r in bbs.keys():
            data[r] = bbs[r]
...
def update():
    try:
        ...
        with urlopen(bbs8_url) as url:
            bbs1 = json.loads(url.read().decode())
        for r in bbs1.keys():
            data1[r] = bbs1[r]
...
effectMap = {
    ...
    '150':"[[Additional Damage Reduction|DoT Mitigation]]",
    ...
    '10005':"[[Turn Skip]]",
    '10511':"[[Doom]]",
    '10550':"[[Active Healing Reduction]]"
}
...
def searchBuffs(bbID, level, procID, extraArg):
    global errors
    arrCount = -1
    for r in data[str(bbID)]['levels'][level]['effects']:
        arrCount = arrCount + 1
...
def printBuffs(unitID, skill):
    ...
    globalArr.clear()
    globalArrCount = -1
    for r in data[str(unitID)]['levels'][skill]['effects']:
        globalArr.append(0)
        try:
...
def findErrors():
    ...
    for unit in data.keys():
        if len(str(unit)) == 7 and unit[0] == '3' and (unit[1] == '7'):
                print("|" + str(data[unit]['name']) + "=")
                printBuffs(unit, 0)
...
#findErrors()
#printBuffs(dbbFindName('3850628'),0)